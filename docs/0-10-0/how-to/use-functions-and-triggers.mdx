...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  iii.register_function(
    RegisterFunctionMessage::with_id("math::add".into()).with_description("Add two numbers and store result".into()),
    move |input| {
...
let iii_clone = iii.clone();
iii.register_function(
  RegisterFunctionMessage::with_id("math::calculate".into()),
  move |input| {
    let iii = iii_clone.clone();
    async move {
...
let iii_clone = iii.clone();
iii.register_function(
  RegisterFunctionMessage::with_id("math::calculate-async".into()),
  move |input| {
    let iii = iii_clone.clone();
    async move {
...
Enqueue work to a named queue for async processing. The engine acknowledges with `{ messageReceiptId }` when the job is accepted. The target function receives the payload when a worker processes it. Requires queues defined in `iii-config.yaml` — see [Use Queues](./use-queues).
...
let iii_clone = iii.clone();
iii.register_function(
  RegisterFunctionMessage::with_id("orders::create".into()),
  move |input| {
    let iii = iii_clone.clone();
    async move {
...
iii.register_function(
  RegisterFunctionMessage::with_id("math::multiply".into()),
  |req| async move {
    let logger = Logger::new();
...
let iii_clone = iii.clone();
iii.register_function(
  RegisterFunctionMessage::with_id("math::aggregation".into()),
  move |_| {
    let iii = iii_clone.clone();
    async move {
      let logger = Logger::new();
...
let iii_clone = iii.clone();
iii.register_function(
  RegisterFunctionMessage::with_id("math::double".into()),
  move |input| {
    let iii = iii_clone.clone();
    async move {
