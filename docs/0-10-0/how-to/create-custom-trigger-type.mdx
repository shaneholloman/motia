...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    iii.register_function(
        RegisterFunctionMessage::with_id("github::push".into()),
        |payload| async move {
...
Before creating a custom type, check the [built-in trigger types](./use-functions-and-triggers#trigger-types) — `http`, `cron`, `queue`, `subscribe`, `state`, and `stream` cover the most common event sources.
</Info>
