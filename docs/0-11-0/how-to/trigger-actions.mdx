...
`TriggerAction.Enqueue({ queue: 'name' })` routes the invocation through a **named queue** configured in `iii-config.yaml`, check how to [create queues in more detail](./use-named-queues). The caller receives an acknowledgement (`messageReceiptId`) once the engine accepts the job but does not wait for it to be processed. The queue provides retries, concurrency control, backoff, and optional FIFO ordering. If all retries are exhausted, the job moves to a dead letter queue.
...
iii.registerFunction('users::register', async (req) => {
...
fn = iii.register_function("users::register", register_user)
```
</Tab>
<Tab title="Rust">
```rust
...
let iii_clone = iii.clone();
let reg = RegisterFunction::new_async("users::register", move |req: Value| {
    let iii = iii_clone.clone();
    async move {
...
iii.registerFunction('etl::extract', async () => {
  const logger = new Logger()
...
fn = iii.register_function("etl::extract", extract)
...
let iii_clone = iii.clone();
let reg = RegisterFunction::new_async("etl::extract", move |_: Value| {
    let iii = iii_clone.clone();
    async move {
        let logger = Logger::new();
...
let iii_clone = iii.clone();
let reg = RegisterFunction::new_async("checkout::process", move |cart: Value| {
    let iii = iii_clone.clone();
    async move {
        let logger = Logger::new();
...
<CardGroup cols={2}>
  <Card title="Use Named Queues" href="./use-named-queues" icon="bolt">
  ...
  <Card title="Trigger Types" href="../architecture/trigger-types" icon="diagram-project">
