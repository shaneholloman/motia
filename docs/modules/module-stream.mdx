...
const fn = iii.registerFunction({ id: 'onJoin' }, (input: StreamJoinLeaveEvent) => {
...
const fn = iii.registerFunction({ id: 'onChatMessage' }, (input: StreamChangeEvent) => {
  console.log(`[${input.type}] ${input.streamName}/${input.groupId}/${input.id}`, input.event.data)
  return {}
})
...
def on_chat_message(input: StreamChangeEvent):
    print(f"[{input.type}] {input.streamName}/{input.groupId}/{input.id}", input.event['data'])
    return {}

iii.register_function({'id': 'onChatMessage'}, on_chat_message)
iii.register_trigger({
    'type': 'stream',
...
<Tab title="Rust">
```rust
use iii_sdk::{IIITrigger, StreamCallRequest, StreamTriggerConfig};
...
iii.register_function((RegisterFunctionMessage::with_id("onChatMessage".into()), |input| async move {
    ...
    println!("[{}] {}/{}/{}: {:?}", event.event_type, event.stream_name, event.group_id, event.id.as_deref().unwrap_or(""), event.event);
    Ok(json!({}))
});
