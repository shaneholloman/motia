...
Spawns external processes (SDK workers) and watches for file changes. Useful for setups where workers need to run alongside the engine on the same host, or to keep external framework workers in sync with the engine lifecycle.
...
```yaml
# Example: run a Python worker
- name: iii-exec
  config:
    watch:
      - ./src
    exec:
      - uv run python -m my_worker

# Example: run a Node.js worker
- name: iii-exec
  config:
    watch:
      - ./src
