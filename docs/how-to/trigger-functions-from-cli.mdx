...
<Steps>
  ...
  <Step title="Identify the function ID">
    ...
    - `iii::durable::publish` — the builtin topic-based publish function
  </Step>
...
Use the builtin `iii::durable::publish` function to publish a message to a topic:

```bash
iii trigger \
  --function-id='iii::durable::publish' \
