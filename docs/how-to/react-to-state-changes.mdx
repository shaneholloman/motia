...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let reg = RegisterFunction::new_async("orders::on-status-change", |event: Value| async move {
...
<Info title="See also">
  For advanced state operations and adapter options, see the [State worker reference](/workers/iii-state).
</Info>
